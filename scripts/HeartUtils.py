...
def writeStats (url, db, ok, now, latency, beatDiff, beat, statsLatency, checkLatency, modifiedLatency):
    if (ok == 1):
        command = "curl -sk -XPOST '%s/write?db=%s' -u $INFLUX_CREDS --data-binary \"hearbeat ok=%d,latency=%f,beatdiff=%d,beat=%d,statsLatency=%f,checkLatency=%s,modifiedLatency=%f %d\"" % (url, db, ok, latency, beatDiff, beat, statsLatency, checkLatency, modifiedLatency, now)
    else:
